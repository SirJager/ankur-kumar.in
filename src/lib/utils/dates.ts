...
export const parseDate = (toISOString: string) => {
	const splits = toISOString.split("T");
	if (splits.length < 2) throw new Error("invalid date format");
	...
	return {
		...
		thisMonth: today.getMonth() + 1 === Number.parseInt(dateComponents[1]),
